...
int main() {
    ...
    std::pair<int, int> gr = graph.randomize(10, Constants::edgeDensity);
    ...
    std::vector<int> vt6 = LinearRounding::solve(graph);
    std::cout << "Linear Rounding: " << vt6 << '\n' << "Norm = " << graph.getNorm(vt6) << '\n';
    return 0;
}